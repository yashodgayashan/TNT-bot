 ...
 #include "indicators.h"
  ...
  #define EMITTER_PIN             12  // emitter is controlled by digital pin 50
  ...
  void calibrateSensorArray(void){
      buzzerBlink(500);                    // Make beep sound before Start calibrate
      for (int i = 0; i < 250; i++)  // make the calibration take about 10 seconds
    {
      qtra.calibrate();             // reads all sensors 10 times at 2.5 ms per six sensors (i.e. ~25 ms per call)
    }
      buzzerBlink(500);                     // Make beep sound after calibrate
  }
